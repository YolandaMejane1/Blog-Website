...
const EditPost = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await axios.get(`https://blog-website-7w3k.onrender.com/api/posts/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`https://blog-website-7w3k.onrender.com/api/posts/${id}`, updatedPost);
      navigate(`/post/${id}`);
    } catch (error) {
